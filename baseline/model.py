...
class Interactions:
    ...
    def interaction_features(self, items):
        ...
        for i_type in range(6):
            ...
            titletags = set(self.item.title)
            for i in interacted_with:
                features[0 + 12*i_type] += len(itemtags.intersection(items[i].tags))/(len(itemtags.union(items[i].tags))+1)
                features[1 + 12*i_type] += len(itemtitle.intersection(items[i].tags))/(len(itemtitle.union(items[i].tags))+1)
                features[2 + 12*i_type] += len(itemtags.intersection(items[i].title))/(len(itemtags.union(items[i].title))+1)
                features[3 + 12*i_type] += len(itemtitle.intersection(items[i].title))/(len(itemtitle.union(items[i].title))+1)
                ...
                features[10 + 12*i_type] += ((items[i].clevel - self.item.clevel)/10)+0.5
                features[11 + 12*i_type] += self.distance((self.item.lat, self.item.lon), (items[i].lat, items[i].lon))
            for f in range(12*i_type, 12*i_type+12):
                features[f] /= length
            features[72+0+12*i_type] = len(itemtags.intersection(items[interacted_with[0]].tags))/(len(itemtags.union(items[interacted_with[0]].tags))+1)
            features[72+1+12*i_type] = len(itemtags.intersection(items[interacted_with[0]].title))/(len(itemtags.union(items[interacted_with[0]].title))+1)
            features[72+2+12*i_type] = len(itemtitle.intersection(items[interacted_with[0]].tags))/(len(itemtitle.union(items[interacted_with[0]].tags))+1)
            features[72+3+12*i_type] = len(itemtitle.intersection(items[interacted_with[0]].title))/(len(itemtitle.union(items[interacted_with[0]].title))+1)
            ...
            features[72+10+12*i_type] = ((items[i].clevel - self.item.clevel)/10)+0.5
            features[72+11+12*i_type] = self.distance((self.item.lat, self.item.lon), (items[interacted_with[0]].lat, items[interacted_with[0]].lon))
    ...
    def onehot_features(self):
        ...
        if self.item.lat in lat:
            f_list[182+self.lat[self.item.lat]] = 1
        if self.item.lon in lon:
